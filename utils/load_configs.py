...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--num_neg_edges', type=int, default=1, help='how many negative edges are used for evluation.')
    # TAMI hyperparameters:
    parser.add_argument('--gamma', type=float, default=0.9, help='gamma hyperparameter used in TRC memory updator.')
    parser.add_argument('--not_load_trc_memory', action='store_true', default=False, help='whether to load the TRC memory')
    # we observe that for certain datasets, not loading TRC memory at the begining of the test stage would result in better performance
    # than loading the TRC memory.
    # Drifting regularizer (auxiliary loss on node temporal embeddings)
    parser.add_argument('--drift_weight', type=float, default=0.0, help='weight of drifting loss (0 disables it)')
    parser.add_argument('--drift_temperatures', type=str, default='0.02,0.05,0.2', help='comma-separated temperatures, e.g. 0.02,0.05,0.2')
    ...
    # Parse drifting temperatures string into a float list
    try:
        args.drift_temperatures = [float(x) for x in args.drift_temperatures.split(',') if x.strip() != '']
    except Exception as e:
        raise ValueError(f'Invalid --drift_temperatures: {args.drift_temperatures}') from e
