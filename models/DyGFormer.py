...
class DyGFormer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.time_encoder = TimeEncoder(time_dim=time_feat_dim, parameter_requires_grad=True)   # using our LTE time encoding function
    ...
    def get_features(self, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray, padded_nodes_edge_ids: np.ndarray,
        ...
        # known_mask = torch.from_numpy(padded_nodes_neighbor_ids != 0).to(self.device)
        # padded_nodes_neighbor_time_features = time_encoder(timestamps=torch.from_numpy(node_interact_times[:, np.newaxis] - padded_nodes_neighbor_times).float().to(self.device), known_mask=known_mask)
