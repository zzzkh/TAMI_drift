...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   ...
                                   num_neighbors: int = 20, time_gap: int = 2000, number_of_negative_edges=1, device=None):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            d_batch_node_interaction_time = torch.tensor(batch_node_interact_times, device=device).reshape(-1, 1)
            negative_logits_list = []
            for i_dx in range(number_of_negative_edges):
                ...
                negative_logits = model[1](src_ids = batch_neg_src_node_ids,
                                                  dst_ids = batch_neg_dst_node_ids,
                                                  src_emb=batch_neg_src_node_embeddings,
                                                  dst_emb=batch_neg_dst_node_embeddings).squeeze(dim=-1)
                negative_logits_list.append(negative_logits)

            # positive_probabilities = model[1](input_1=batch_src_node_embeddings, input_2=batch_dst_node_embeddings).squeeze(dim=-1).sigmoid()
            positive_logits = model[1](src_ids = batch_src_node_ids, 
            ...
            negative_logits_cat = torch.cat(negative_logits_list, dim=0)
            logits = torch.cat([positive_logits, negative_logits_cat], dim=0)
            labels = torch.cat([torch.ones_like(positive_logits), torch.zeros_like(negative_logits_cat)], dim=0)
