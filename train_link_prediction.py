...
from models.modules import MergeLayer, HistoricalDecoder
...
if __name__ == "__main__":
    ...
    class EmbeddingRingBuffer:
        """A fixed-size GPU ring buffer for storing positive embeddings without autograd history."""

        def __init__(self, max_size: int, dim: int, device: str):
            self.max_size = int(max_size)
            self.dim = int(dim)
            self.device = device
            self.buf = torch.empty((self.max_size, self.dim), device=device)
            self.ptr = 0
            self.filled = 0
        ...
        def append(self, x: torch.Tensor) -> None:
            ...
            if n <= 0:
                return
            if n >= self.max_size:
                self.buf.copy_(x[-self.max_size :])
                self.ptr = 0
                self.filled = self.max_size
                return
            ...
            if end <= self.max_size:
                ...
                k = self.max_size - self.ptr
                self.buf[self.ptr :].copy_(x[:k])
                self.buf[: end - self.max_size].copy_(x[k:])

            self.ptr = end % self.max_size
            self.filled = min(self.max_size, self.filled + n)

        def sample(self, n: int) -> Optional[torch.Tensor]:
            if self.filled <= 0:
                return None
            n = min(int(n), int(self.filled))
            if n <= 0:
                return None
            idx = torch.randint(0, self.filled, (n,), device=self.device)
            return self.buf[idx]
    ...
    for run in range(args.num_runs):
        ...
        link_predictor = HistoricalDecoder(input_dim1=node_raw_features.shape[1], input_dim2=node_raw_features.shape[1],
                                    hidden_dim=node_raw_features.shape[1], output_dim=1, gamma=args.gamma, device=args.device)
        ...
        for epoch in range(args.num_epochs):
            ...
            # NOTE: IMPORTANT for TRC module: 
            # reset memory at the start of each training epoch
            # otherwise would cause test data leakage.
            model[1].historical_interaction_memory.reset_memory()
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                d_batch_node_interaction_time = torch.tensor(batch_node_interact_times, device=args.device).reshape(-1, 1)
                positive_logits = model[1](
                                                  src_ids = batch_src_node_ids,
                                                  dst_ids = batch_dst_node_ids,
                                                  src_emb=batch_src_node_embeddings,
                                                  dst_emb=batch_dst_node_embeddings,
                                                  update_memories=True).squeeze(dim=-1)
                negative_logits = model[1](
                                                  src_ids = batch_neg_src_node_ids,
                                                  dst_ids = batch_neg_dst_node_ids,
                ...
                drift_loss = torch.tensor(0.0, device=args.device)
                if args.drift_weight > 0:
                    if pos_dst_bank is None:
                        pos_dst_bank = EmbeddingRingBuffer(
                            max_size=args.drift_bank_size,
                            dim=batch_dst_node_embeddings.shape[1],
                            device=args.device,
                        )
                    ...
                    if y_pos is not None:
                        ...
                        V = compute_V_multi_temperature(
                            ...
                            y_pos=y_pos,
                            y_neg=y_neg,
                            temperatures=args.drift_temperatures,
                            mask_self=False,
                            normalize_each=True,
                        )
                        drift_loss = (V ** 2).mean()
            ...
            # backup decoder memory bank after training so it can be used for new validation nodes
            train_dec_memory_bank = model[1].historical_interaction_memory.backup_memory_bank()
            ...
            val_losses, val_metrics = evaluate_model_link_prediction(model_name=args.model_name,
                                                                     ...
                                                                     loss_func=loss_func,
                                                                     num_neighbors=args.num_neighbors,
                                                                     time_gap=args.time_gap, device=args.device)
            ...
            # backup decoder memory bank after validating so it can be used for testing nodes (since test edges are strictly later in time than validation edges)
            val_dec_memory_bank = model[1].historical_interaction_memory.backup_memory_bank()
            # reload decoder training memory bank for new validation nodes
            model[1].historical_interaction_memory.load_memory_bank(train_dec_memory_bank)
            ...
            for metric_name in val_metrics[0].keys():
                plot_val_eval_metrics[metric_name].append(np.mean([val_metric[metric_name] for val_metric in val_metrics]))
            # ==========================================================
            ...
            if (epoch + 1) % args.test_interval_epochs == 0:
                ...
                if args.model_name in ['JODIE', 'DyRep', 'TGN']:
                    ...
                    model[0].memory_bank.reload_memory_bank(val_backup_memory_bank)
                # reload decoder validation memory bank for new testing nodes
                model[1].historical_interaction_memory.load_memory_bank(val_dec_memory_bank)
                ...
                if args.model_name in ['JODIE', 'DyRep', 'TGN']:
                    ...
                    # note that since model treats memory as parameters, we need to reload the memory to val_backup_memory_bank for saving models
                    model[0].memory_bank.reload_memory_bank(val_backup_memory_bank)
                # reload decoder validation memory bank for saving models
                model[1].historical_interaction_memory.load_memory_bank(val_dec_memory_bank)
        ...
        if args.model_name in ['JODIE', 'DyRep', 'TGN']:
            ...
            val_backup_memory_bank = model[0].memory_bank.backup_memory_bank()
        # the memory in the best model has seen the validation edges, we need to backup the decoder memory for new testing nodes
        val_dec_backup_memory = model[1].historical_interaction_memory.backup_memory_bank()
        ...
        if args.model_name in ['JODIE', 'DyRep', 'TGN']:
            ...
            model[0].memory_bank.reload_memory_bank(val_backup_memory_bank)
        # reload decoder validation memory bank for new testing nodes
        model[1].historical_interaction_memory.load_memory_bank(val_dec_backup_memory)
